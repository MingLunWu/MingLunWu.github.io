...
<img style="display:block; margin-left:auto; margin-right:auto; width:100%;" src="https://minglunwu.github.io/images/20220304/pytest_3.png"><br>

這邊出現了一個有意思的狀況: 當我們丟 `int` 格式給 `concat()` 時，我們**期待它要拋出錯誤**，現在它真的拋出錯誤了，在測試中到底要是成功還是失敗呢？

為了要確保「錯誤功能」有正確運作，我們可以把剛剛的測試函式改成:
...
<img style="display:block; margin-left:auto; margin-right:auto; width:100%;" src="https://minglunwu.github.io/images/20220304/pytest_4.png"><br>
...
<img style="display:block; margin-left:auto; margin-right:auto; width:100%;" src="https://minglunwu.github.io/images/20220304/pytest_5.png"><br>
...
快速的檢查當前的 Test Case 是否正確，可以更頻繁的進行迭代。
...
發現了嗎？ 因為三個函式都需要對 Dictionary 進行操作，所以三個 Test Case 的一開始都需要宣告一個 `test_dict`，但其實他們是一樣的東西。
...
在 `fixture.py` 中，我們將會被重複使用的物件封裝為`fixture`，具體來說是用一個函式來回傳結果，並在該函式上加上一個 `@pytest.fixture` 裝飾子。
