...
在上圖 yml 檔中，`task-2 (show file context)` 使用 command 來顯示特定位置的檔案。
...
除了單純使用 `command` 下語法外， `Ansible` 還有提供許多內建的語法，讓使用者能用簡單的語法執行複雜的任務（例如檔案新增、權限變動、Git下載等等)。
...
透過 `Module - lineinfile` ， `task1` 在 `/tmp/hello_world.txt` 創建一個檔案。

### 4.3 Variable

在 `Playbook` 中，我們還可以自定義變數，以 `main.yml` 為例: 
...
<img style="display:block; margin-left:auto; margin-right:auto; width:30%;" src="https://minglunwu.github.io/images/20210927/ansible-playbook.png">
